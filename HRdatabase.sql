...
CREATE TABLE locations (
	...
	alter table locations alter column country_id CHAR(5);
...
CREATE TABLE employees (
	...
	alter table employees alter column hire_date varchar(15);
...
CREATE TABLE job_history (
	...
	alter table job_history alter column start_date varchar(15);
	alter table job_history alter column end_date varchar(15);
...
/*ALTER TABLE job_history ADD UNIQUE INDEX (
	...
	select
	 cast(hire_date as date) as h_date from employees;
	select cast(start_date as date)as start_date ,cast(end_date as date) as end_date from job_history
...
WHERE e.department_id = d.department_id
	...
	CREATE VIEW [emp_details_views]
AS
SELECT e.employee_id,cast(hire_date as date) as h_date,
	e.job_id,
	e.manager_id,
	e.department_id,
	d.location_id,
...
select CONCAT(first_name ,' ',Last_name) as First_Name_and_Last_Name from emp_details_views 
...
select employee_id, concat (first_name,' ',last_name)  as names , salary, floor(salary*0.15) as Pf from  employees order by pf ;

6--Write a query to get the highest 1,2,3,salaries payable to employees.
select * from (select employee_id,salary,dense_rank() over(order by salary desc) as Highest_salary
 from employees) as temp where Highest_salary in (1,2,3);
